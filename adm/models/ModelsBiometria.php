...
class ModelsBiometria
{
    ...
    {
        $sql = "SELECT bio.ID as idbio, usu.nome as NOME,SERIAL_NUMBER,PATRIMONIO,DATA_HORA,bio.STATUS,bio.TIPO_EQPTO  FROM biometria bio INNER JOIN usuario usu on usu.ID = bio.FK_ID_USUARIO WHERE TIPO_EQPTO IN (1,2) AND STATUS_EXC = '0' ORDER BY bio.ID DESC LIMIT 0,20";
        $read = new ModelsRead();
        $read->FullRead($sql);
                ...
                <tbody>
                    ...
                    ?>
                        <tr id="rowline<?= $listar['idbio'] ?>">
                            <td scope="row"><?= $listar['idbio'] ?></td>
                            <td>
                                <?php
                                if ($listar['TIPO_EQPTO'] === '1') :
                                ?>
                                    <img src="<?= URL ?>assets/image/biometria.png" style="width:20px;" />
                                <?php
                                else :
                                ?>
                                    <img src="<?= URL ?>assets/image/FOTO2.png" style="width:20px;" />
                                <?php
                                endif;
                                ?>&nbsp;
                            <?= $listar['SERIAL_NUMBER'] ?></td>
